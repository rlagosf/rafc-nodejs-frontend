...
export default function RegistroNoticias() {
  ...
  const openSlot = async (slot) => {
    ...
    if (slot?.item?.id) {
      ...
      try {
        ...
        const archived = isArchivedItem(it); // ✅ ya NO bloqueamos, solo marcamos readOnly
  ...
  const handleSave = async () => {
    if (saving) return;
    if (form.readOnly) return; // ✅ blindaje extra
  ...
  const badgeEstadoClass = (id) => {
    ...
    if (name.includes("archiv")) return "bg-red-600 text-white border-red-700 shadow-sm"; // ✅ más vivo
