// src/pages/admin/modulo-financiero/pagosCentralizados.jsx
...
export default function PagosCentralizados() {
  ...
  // 🧭 Breadcrumb por defecto (si no viene desde el dashboard)
  useEffect(() => {
  ...
  const filasFiltradas = useMemo(() => {
    ...
    return filas.filter((row) => {
      ...
      if (filtroEstado) {
        ...
        okEstado = est === filtroEstado.toUpperCase();
      }
  ...
  return (
    <div className={`${fondoClase} min-h-[calc(100vh-100px)] px-4 pt-4 pb-16 font-realacademy`}>
      ...
      <p className="text-center mb-6 text-xs sm:text-sm opacity-80">
        Vista consolidada de todos los pagos registrados en{' '}
        <span className="font-semibold">pagos_jugador</span>. Cada fila corresponde a un pago
        individual, con filtros por estado, tipo, categoría y medio de pago.
      </p>

      {/* Filtros propios del detalle de pagos */}
                  ...
                  <td className="py-2 px-2 border text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        ...
                        title={pago && pago.id ? 'Editar pago' : 'Sin pago para editar'}
                        disabled={!pago || !pago.id}
                      >
                        <Pencil size={18} />
                      </button>
                      <button
                        ...
                        className="p-1 rounded hover:bg-black/10 dark:hover:bg-white/10 disabled:opacity-50"
                        title={pago && pago.id ? 'Eliminar pago' : 'Sin pago para eliminar'}
                        disabled={!pago || !pago.id}
                      >
      ...
      <div className="flex flex-col items-center justify-center gap-2 mt-4">
        ...
        <div className="text-xs opacity-80">
          ...
          <span className="font-semibold">{filasOrdenadas.length}</span> pagos filtrados.
        </div>
      </div>
