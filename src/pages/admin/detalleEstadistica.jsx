...
export default function DetalleEstadistica() {
  ...
  const campos = useMemo(() => ({
    Ofensivas: [
      'goles', 'asistencias', 'tiros_libres', 'penales',
      'tiros_arco', 'tiros_fuera', 'tiros_bloqueados',
      'regates_exitosos', 'centros_acertados', 'pases_clave',
    ],
    Defensivas: [
      'intercepciones', 'despejes', 'duelos_ganados',
      'entradas_exitosas', 'bloqueos', 'recuperaciones',
    ],
    Técnicas: [
  ...
  const blankForm = (eid) => {
    ...
    Object.values(campos).flat().forEach((campo) => {
  ...
  useEffect(() => {
    ...
    if (needsAppend) {
      navigate(currentPath, {
        replace: true,
        state: { ...(location.state || {}), breadcrumb: [...base, { label: 'Detalle Estadística', to: currentPath }] },
      });
    }
  ...
  return (
    <div className={`${fondoClase} px-4 pt-4 pb-16 font-realacademy`}>
